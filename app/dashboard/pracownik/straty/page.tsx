...
export default function EmployeeLossesPage() {
    ...
    async function loadData() {
        try {
            const res = await fetch("http://localhost/hurtownia-api/employee_losses.php", {
                credentials: "include",
            });
            ...
            if (!res.ok || !json.success) {
                setError(json.message || "Błąd pobierania strat");
                return;
            }
                ...
                <aside className="hidden w-72 flex-col justify-between bg-slate-950 px-6 py-8 text-white lg:flex">
                    <div>
                        ...
                        <nav className="space-y-2">
                            ...
                            <button onClick={() => (window.location.href = "/dashboard/pracownik/reklamacje")} className="block w-full rounded-2xl px-4 py-3 text-left text-slate-300 hover:bg-white/5">
                                Reklamacje
                            </button>
                            <button onClick={() => (window.location.href = "/dashboard/pracownik/straty")} className="block w-full rounded-2xl bg-white/10 px-4 py-3 text-left font-medium">
                                Straty
                            </button>
                        ...
                        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-5 lg:px-10">
                            <div>
                                ...
                                <p className="mt-2 text-slate-500">
                                    Lista Twoich strat z możliwością wyszukiwania.
                                </p>
                            </div>

                            <button
                                onClick={() => (window.location.href = "/dashboard/pracownik/straty/dodaj")}
                    ...
                    <section className="mx-auto max-w-7xl px-6 py-8 lg:px-10">
                        <div className="mb-6 rounded-3xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
                            <input
                                ...
                                placeholder="Szukaj po produkcie, statusie, powodzie..."
                            ...
                            <table className="w-full text-left">
                                ...
                                <tbody>
                                    {filtered.map((item) => (
                                        <tr
                                            key={item.id}
                                            onClick={() => (window.location.href = `/dashboard/pracownik/straty/${item.id}`)}
