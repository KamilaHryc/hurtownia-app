...
export default function AdminAddProductPage() {
    ...
    async function submitForm(e: React.FormEvent) {
        ...
        const res = await fetch("http://localhost/hurtownia-api/product_add.php", {
            method: "POST",
    ...
    return (
        <AdminLayout active="Produkty" handleLogout={handleLogout}>
            <Header title="Dodaj produkt" subtitle="Dodaj nowy produkt do bazy." />

            <FormCard>
                <form onSubmit={submitForm} className="space-y-4">
                    <Input label="Nazwa produktu" value={name} setValue={setName} />

                    <div>
                        <label className="mb-2 block text-sm font-bold text-slate-700">Kategoria</label>
                        <select value={category} onChange={(e) => setCategory(e.target.value)} className="w-full rounded-xl border border-slate-300 p-3">
                    ...
                    <div>
                        <label className="mb-2 block text-sm font-bold text-slate-700">Jednostka</label>
                        <select value={unit} onChange={(e) => setUnit(e.target.value)} className="w-full rounded-xl border border-slate-300 p-3">
                            <option value="kg">kg</option>
                            <option value="szt">szt</option>
                        </select>
                    </div>

                    <Input label="Cena" value={price} setValue={setPrice} type="number" />
