...
export default function AdminProductsPage() {
    ...
    async function loadProducts() {
        try {
            const res = await fetch("http://localhost/hurtownia-api/admin_products.php", {
                credentials: "include",
            });
            ...
            if (!res.ok || !json.success) {
                setError(json.message || "Nie udało się pobrać produktów");
                return;
            }
    ...
    async function deleteProduct(product: Product) {
        if (!confirm(`Czy na pewno usunąć produkt ${product.name}?`)) return;

        setMessage("");
        setError("");

        const res = await fetch("http://localhost/hurtownia-api/admin_product_delete.php", {
            method: "POST",
        ...
        if (!res.ok || !json.success) {
            setError(json.message || "Nie udało się usunąć produktu");
            return;
        }
    ...
    async function updateProduct(e: React.FormEvent) {
        ...
        const res = await fetch("http://localhost/hurtownia-api/admin_product_update.php", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            credentials: "include",
    ...
    const filtered = useMemo(() => {
        ...
        return items.filter((item) =>
            [item.id, item.name, item.category, item.unit, item.price]
                .join(" ")
                .toLowerCase()
                .includes(q)
        );
    ...
    return (
        <main className="min-h-screen bg-slate-100">
                ...
                <aside className="hidden w-72 flex-col justify-between bg-slate-950 px-6 py-8 text-white lg:flex">
                    <div>
                        ...
                        <nav className="space-y-2">
                            ...
                            <MenuItem label="Produkty" href="/dashboard/admin/produkty" active />
                        ...
                        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-5 lg:px-10">
                            <div>
                                ...
                                <p className="mt-2 text-slate-500">
                                    Dodawanie, edycja, usuwanie i wyszukiwanie produktów.
                                </p>
                            </div>
            ...
            {editing && (
                <Modal
                    title={`Edytuj produkt: ${editing.name}`}
